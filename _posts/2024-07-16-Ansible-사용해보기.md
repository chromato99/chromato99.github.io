...
여기까지 ansible에 대한 대략적인 소개와 이를 사용해 간단한 홈서버를 구축하는 방법에 대해 알아보았다. 개인적으로 홈서버를 구축할때 사용한 전체 코드를 공유하고 싶지만, 보안에 민감한 정보가 포함되어 있어 공유하지 못하였다. 기본적인 사용법은 위와 같으니 참고하면 자신만의 서버를 구축할 수 있을 것이다.